...
# Table Location/Descriptions - currently 4 tables, winner and losers side
pool_table = ['Table 01 L', 'Table 01 W', 'Table 02 L', 'Table 02 W', 'Table 03 L', 'Table 03 W', 'Table 04 L', 'Table 04 W']

# Players at the Tables - 4 tables, 8 positions, positions 1-8
active_player_tables = []

# Players waiting pool
    ...
    while table_num != "done":
        ...
        if table_num == "chips":
            ...
            if (table_num % 2) == 0: # If on loser location and lost
                ...
                new_player = random.sample(active_players, 1) # Choosing a new player.
                while chips[new_player[0]] < 1: # Checking that new players has chips.
                    new_player = random.sample(active_players, 1)

                active_player_tables[table_num] = new_player[0]
                print("Send team #", new_player[0], "to Table Position:", table_num+1)
                active_players.append(buffer)
                active_players.remove(new_player[0])
                ...
                active_player_tables[table_num - 1] = new_player[0]
                print("Send team #", new_player[0], "to Table Position:", table_num+1)
                active_players.append(buffer)
                active_players.remove(new_player[0])
